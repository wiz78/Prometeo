/***************************************************************************
                                procpool.cpp
                             -------------------
    revision             : $Id: procpool.cpp,v 1.1 2003-05-24 12:28:53 tellini Exp $
...
void ProcPool::Setup( void )
{
	...
	if( MinChildren > Children.Count() ) {
		...
		while( MinChildren > Children.Count() ) {
			...
			if( proc->Spawn( "POP3 proxy" ))
				AddChild( proc );
			else {
				App->Log->Log( LOG_ERR, "mod_pop3: couldn't spawn a new process" );
				delete proc;
			}
		}
	}
...
void ProcPool::ServeClient( TcpSocket *sock )
{
	...
	if( proc )
		...
		sock->Printf( "-ERR Too many clients.\r\n" );
		delete sock;
	}
}
//---------------------------------------------------------------------------
